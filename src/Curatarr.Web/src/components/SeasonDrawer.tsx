...
export const SeasonDrawer: React.FC<SeasonDrawerProps> = ({ seasons, onPruneSeason }) => {
  ...
  return (
    <div className="bg-slate-950/80 border-t border-slate-800/80 p-2 space-y-1.5 rounded-b-xl">
      ...
      <div className="space-y-1">
        {seasons.map((s) => (
          <div
            key={s.id}
            className="flex items-center justify-between bg-slate-900/60 border border-slate-800 px-2 py-1 rounded-md text-[11px]"
          >
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="font-mono text-[10px] text-slate-400">{formatSize(s.sizeBytes)}</span>
              <button
                onClick={(e) => {
                  e.stopPropagation();
